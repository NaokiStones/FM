...
public class SGD {
    ...
    private double eta = 0.005; // reg0, number:by intuition
    ...
    private double predict(double y, Map<Integer, Double> record) {
        ...
        if(Double.isNaN(ret)) {
            System.out.println("Predict:NaN!");
            System.exit(1);
        }
        //********************************
        return ret;
    ...
    public OutputData learn(InputData id, Target tg, int k, String task) {
        ...
        for(int iter = 0; iter < 30; iter++) { // tmp
            ...
            for(int p = 0; p < id.getRow(); p++, t++) {
            	...
            	double error = predict(y, record);
                double grad0 = calcGrad(0, record, y, "w0");
                ada0Grad = 1 /*(grad0 * grad0)*/;	// adaGrad for w0  temp
                w0 = w0 - eta * (/* ada0() * */ (grad0)  + 2 * lambda0 * w0);
                ...
                for(int i : record.keySet()) {
                    ...
                    adaWGrad[i] += (gradWi * gradWi);	// adaGrad for w[i]
                    int pi = pi(i);
                    double nextWi = w[i] - eta* (/*adaW(i)* */(gradWi)  + 2 * lambdaW.get(pi) * w[i]);
                    w[i] = nextWi;
                    
                    //System.out.println("gradWi:" + gradWi);
                    
                    for(int f = 0; f < k; f++) {
                        double gradVij = dloss(predict(y, record), y, i, f, "v", record);
                        adaVGrad[i][f] += (gradVij * gradVij);	// adaGrad for V[i][f]
                        
                        //System.out.println("gradVij:" + gradVij); //****
